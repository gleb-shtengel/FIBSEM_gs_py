...
def get_min_max_thresholds(image, **kwargs):
    ...
    db = (bins[1]-bins[0])/2.0
    data_max = bins[np.argmin(np.abs(cdf-(1.0-thr_max)))] + db
    data_min = bins[np.argmin(np.abs(cdf-thr_min))] + db
...
def calculate_image_contrast(image_array, **kwargs):
    ...
    ax.plot(x, pdf, color = 'red', linewidth = 3, label='PDF data')
    ax.plot(x, gauss_fit2, color = 'lime', linewidth = 3, label='Double Gauss Fit')
    ax.plot(x, gauss_fit_low, color = 'cyan', linewidth = 1, label='Low Gauss Fit')
    ax.plot([I_low, I_low], [0, Pmax], color = 'cyan', linestyle='dashed', label='$I_{low}$' +' = {:.1f}'.format(I_low))
    ax.plot(x, gauss_fit_high, color = 'magenta', linewidth = 1, label='High Gauss Fit')
    ax.plot([I_high, I_high], [0, Pmax], color = 'magenta', linestyle='dashed', label='$I_{high}$' +' = {:.1f}'.format(I_high))
    ax.plot(x[0], pdf[0], color = 'black', linestyle='none',  label='$I_{0}$' +' = {:.1f}'.format(I0))
    ax.plot(x[0], pdf[0], color = 'black', linestyle='none',  label='Image Contrast =  {:.3f}'.format(contrast))
...
def determine_sizes_and_offsets(shapes, tr_matr):
    ...
    xmin = np.min(np.round(np.concatenate((xc, zeros)))).astype(int)
    xmax = np.max(np.round(np.concatenate((xc, xszs))).astype(int))
    ymin = np.min(np.round(np.concatenate((yc, zeros)))).astype(int)
    ymax = np.max(np.round(np.concatenate((yc, yszs))).astype(int))
    #print(xmin, xmax, ymin, ymax)
    xi = np.max(-xmin)
    yi = np.max(-ymin)
    xsz = np.max(xmax - xmin)
    ysz = np.max(ymax - ymin)
    #print(xi, yi, xsz, ysz)
