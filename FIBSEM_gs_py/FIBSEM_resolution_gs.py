...
def analyze_blob_transitions(amp, ** kwargs):
    ...
    if disp_res and ax:
        ...
        ax.plot([rise_points[0], rise_points[1]], [rise_points[2], rise_points[3]], 'o', color = col, markersize = markersize,
            label = '{:.0f}%-{:.0f}%: {:.2f}nm'.format(bounds[0]*100, bounds[1]*100, abs(rise_points[1]-rise_points[0])))
        ax.plot([fall_points[0], fall_points[1]], [fall_points[2], fall_points[3]], 's', markersize = markersize, color = col,
                label = '{:.0f}%-{:.0f}%: {:.2f}nm'.format(bounds[0]*100, bounds[1]*100, abs(fall_points[1]-fall_points[0])))
        ...
        if len(blobs_LoG_list)>0:
            ...
            if len(error_flags[error_flags==0]) > 0:
                ...
                if save_data_xlsx:
                    ...
                    if save_good_blobs_only:
                        if verbose:
                            print(time.strftime('%Y/%m/%d  %H:%M:%S') + ' Saving only good blob data')
                        ...
                        if verbose:
                            print(time.strftime('%Y/%m/%d  %H:%M:%S') + ' Saving all blob data')
